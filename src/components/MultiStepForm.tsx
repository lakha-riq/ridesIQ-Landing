...
const trackingTypeOptions = [
  { icon: Bus, title: 'Vans / Trucks', value: 'vans-trucks' },
  ...
  { icon: Caravan, title: 'Trailers', value: 'trailers' },
  { icon: Package, title: 'Non-Powered Assets', value: 'non-powered' },
];
...
export const MultiStepForm: React.FC = () => {
  ...
  const renderNavigationButtons = () => (
    <div className="shrink-0 flex justify-between items-center pt-6 border-t border-gray-100">
      {step >= 2 ? (
        <button
          onClick={handleBack}
                ...
                {interestOptions.map((option) => (
                  ...
                  >
                    <div className="flex flex-row items-start justify-center gap-2">
                      <option.icon className="w-6 h-6 sm:w-8 sm:h-8 text-[#678FCA]" />
                      <span className="text-base sm:text-lg font-medium text-start leading-snug">
                        {option.title}
                      </span>
                    </div>
                  </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {featureOptions.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => {
                      const newFeatures = formData.features.includes(
                        option.value
                      )
