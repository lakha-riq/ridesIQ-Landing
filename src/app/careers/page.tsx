...
import { Sparkles, Rocket, Home, TrendingUp, DollarSign, Heart, Users, Code, LineChart, Headphones, Database, ChevronRight, Search, MapPin, Briefcase, Clock } from "lucide-react";
...
const jobListings = [
	{
		...
		location: "Toronto, ON",
		type: "Full-time",
		description: "Build and maintain our core fleet management platform using modern technologies.",
		remote: true,
	},
	{
		id: 2,
...
const Careers = () => {
	...
	const filteredJobs = jobListings.filter((job) => {
		...
		const matchesDepartment = selectedDepartment.value === "all" || job.department.toLowerCase() === selectedDepartment.value;
		const matchesLocation = selectedLocation.value === "all" || (selectedLocation.value === "remote" ? job.remote : job.location.toLowerCase().includes(selectedLocation.value));
		const matchesType = selectedType.value === "all" || job.type.toLowerCase() === selectedType.value;
					...
					<div className="mb-8 space-y-4">
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							<Select
								...
								onChange={(option) => setSelectedDepartment(option || departments[0])}
								className="text-base"
							...
							<Select
								...
								onChange={(option) => setSelectedLocation(option || locations[0])}
								className="text-base"
							...
							<Select
								...
								value={selectedType}
								onChange={(option) => setSelectedType(option || employmentTypes[0])}
								className="text-base"
					...
					<div className="space-y-4">
						{filteredJobs.length > 0 ? (
										...
										<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
											<div>
												...
												<div className="flex flex-wrap gap-2">
													...
													{job.remote && <span className="inline-flex items-center px-3 py-1 rounded-full bg-green-100 text-green-600 text-sm">Remote Available</span>}
												</div>
											</div>
							...
							<div className="text-center py-12">
								<p className="text-gray-600">No positions available matching your criteria.</p>
							</div>
						)}
					</div>
				</div>
						...
						{teams.map((team, index) => (
							...
							>
								...
								<div className="relative bg-white rounded-2xl p-8 shadow-lg transition-all duration-300 group-hover:shadow-xl border border-gray-100/50 text-center">
									...
									<h3 className="text-xl font-semibold mb-2 text-gray-900">{team.title}</h3>
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
					...
					<div className="max-w-4xl mx-auto">
						<div className="relative">
							...
							{[
								{ step: 1, title: "Find Your Role", description: "Browse our open positions and find the perfect match for your skills." },
								{ step: 2, title: "Submit Your Application", description: "Upload your resume & cover letter through our online portal." },
								{ step: 3, title: "Initial Screening", description: "Our recruitment team will review your application." },
								{ step: 4, title: "Interview Process", description: "Technical/HR interviews with the team." },
								{ step: 5, title: "Offer", description: "Successful candidates will receive a formal offer." },
							].map((step, index) => (
								<motion.div
									key={step.step}
								...
								>
									<div className="flex-shrink-0 w-24 h-24 rounded-2xl bg-gradient-to-br from-[#678FCA] to-[#99D5C9] p-0.5">
										<div className="w-full h-full rounded-2xl bg-white flex items-center justify-center text-2xl font-bold text-[#678FCA]">{step.step}</div>
									</div>
